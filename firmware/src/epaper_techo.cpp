...
void epdEsperaPintado(uint32_t minimoMs, uint32_t topeBusyMs) {
  ...
  const uint32_t gastado = millis() - t0;
  if (gastado < minimoMs) delay(minimoMs - gastado);
}
...
//   accion NO se ejecuta a la primera pulsacion larga; se pide confirmacion y solo la
//   SEGUNDA pulsacion larga de la MISMA accion dentro de la ventana la lleva a cabo.
...
bool menuSave(const char *key, const char *val) {
  ...
  if (ok) { storeSave(*gCfg); Serial.printf("MENU %s=%s ok\r\n", key, val?val:""); }
...
//   pulsar: una carrera que se pierde. El patron es el mismo (segunda pulsacion larga de la
...
static bool esAccionDestructiva(int act) {
  return act == ACT_SLEEP || act == ACT_REBOOT || act == ACT_RESET ||
...
static void menuEjecutaAccion(int act) {
  ...
  // ★ PRIMERA pulsacion larga: NO se ejecuta, se PIDE CONFIRMACION.
  //   SEGUNDA pulsacion larga de la MISMA accion dentro de la ventana: se ejecuta.
...
void menuNavigate() {
  if (!gMenuOn) return;
  gMenuLastActMs = millis();
  gUltimoToqueMs = millis();   // aplaza el repintado si vas tocando rapido (2026-09-15)
  ...
  // pedido la confirmacion. Se sale confirmando (largo) o dejando vencer la ventana.
...
void menuPinta() {
  ...
  //   se confirme, se cancele con otra pulsacion larga o venza la ventana, con la misma letra
  //   y el mismo estilo que el resto del menu:
  //       "CONFIRMAR"                          (escala 1, arriba)
  //       [nombre de la accion]                (escala 2, dentro de un recuadro)
  //       "pulsa largo otra vez"               (escala 1)
  //       "para confirmar"
  ...
  if (gMenuOn && confirmacionPendiente()) {
    ...
    if (tit) {
      ...
      drawTextCenter(120, "pulsa largo otra vez", 1);
...
void displayNextScene() {
  ...
  gUltimoToqueMs = millis();     // toque reciente: aplaza el repintado (toque rapido)
...
void displayRefresh(const DigiConfig &cfg, uint32_t rxCount, uint32_t txCount,
  ...
  // ★ TOQUE RAPIDO (2026-09-15): si acabas de pulsar hace menos de kDebounceToqueMs, se
  //   APLAZA el repintado (se mantiene gDirty sin pintar). Asi, si tocas varias veces
  //   seguidas en el carrusel o el menu, la posicion LOGICA avanza al instante en cada toque
  //   y solo se pinta UNA vez cuando dejas de tocar (a los ~400 ms), mostrando la posicion
  //   final. Es lo que hace que el menú y las diapositivas respondan "libre" al pulso rapido.
  const bool toqueReciente = (ahora - gUltimoToqueMs) < kDebounceToqueMs;
