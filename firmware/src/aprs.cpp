...
// The operator's rule (2026-09-13): a tracker must NEVER publish the configured
// point. If the GPS has a live fix, that position goes out. If it does not, the
// LAST KNOWN position is sent instead (the most recent real fix, remembered
// across reboots, with its own time and the ">ULTIMA CONOCIDA" marker). Only
// mode 0 (fixed digipeater) publishes the configured point, because for a fixed
// station that point IS its position.
// Returns -111 when there is nothing real to send (no fix and nothing
// remembered): silence is better than a made-up position.
int16_t aprsSendManualBeacon(const DigiConfig &cfg) {
  if (cfg.mode != 0) {
    ...
    if (lastPosFill(last, fixTimeValid)) {
      return aprsSendTrackerBeacon(cfg, last, 'M', nullptr, true, true);
    }
    return -111;  // no fix and nothing remembered
  }
