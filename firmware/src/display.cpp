...
/* ============================================================================
   ...
   ============================================================================ */
constexpr int kRowH = 12;      // alto de una banda de texto
...
void headerBar(const String &text) {
  const int pad = 4;
  const int x0 = 2;
...
// Bottom page indicator (Meshtastic style): one small rectangle per scene,
// the current one filled. Left-aligned to leave room for the battery on the
// bottom right.
void footerDots() {
  const int dotW = 7, gap = 2;
  int x0 = 2;
  for (int i = 0; i < kSceneCount; i++) {
    int x = x0 + i * (dotW + gap);
    if (i == gScene) {
      gOled->fillRect(x, 58, dotW, 4);
    } else {
      gOled->drawRect(x, 58, dotW, 4);
    }
  }
}
...
void drawBatteryFooter() {
  ...
  int tx = 127 - tw;
  gOled->drawString(tx, 54, t);

  int bx = tx - 19;   // battery body just left of the text
  int by = 57;
  gOled->drawRect(bx, by, 14, 7);
  gOled->fillRect(bx + 14, by + 2, 2, 3);  // + terminal
